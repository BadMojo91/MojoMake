...
void DeusExBuilder::backupAndRemoveUFiles(const std::vector<std::string>& packages, bool ue2) {
	for (const auto& pkg : packages) {
		if (ue2) {
			...
			std::string ued22x_u_file = ued22x_dir + "\\" + pkg + ".u";
			if (fs::exists(ued22x_u_file)) {
				std::cout << "Backing up and removing " << pkg << ".u from UED22x..." << std::endl;
				fs::copy_file(ued22x_u_file, ued22x_u_file + ".bak", fs::copy_options::overwrite_existing);
				fs::remove(ued22x_u_file);
			}
		}
		else {
...
void DeusExBuilder::moveCompiledFiles(const std::vector<std::string>& packages, bool ue2) {
	for (const auto& pkg : packages) {
		if (ue2) {
			...
			std::string ued_pkg = "\\" + pkg + ".u";
			std::string ued22_file = ued22_dir + ued_pkg;
			std::string ued22x_file = ued22x_dir + ued_pkg;
			if (fs::exists(ued22_file)) {
				std::cout << "Compiled " << ued22_file << std::endl;

				if (ue22x_support) {
					std::cout << "Copying " << pkg << ".u to UED22x folder..." << std::endl;

					fs::copy(ued22_file, ued22x_file, fs::copy_options::overwrite_existing);
					
					//	std::cerr << "Failed to copy " << pkg << ".u to UED22x folder." << std::endl;
					

					if (fs::exists(ued22x_file)) {
						std::cout << "Copied " << pkg << ".u to UED22x folder." << std::endl;
					}
				}
			}
		}
